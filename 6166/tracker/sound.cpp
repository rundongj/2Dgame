  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/toggle.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load toggle.mp3")+Mix_GetError();

  startMusic();

  sounds.push_back( Mix_LoadWAV("sound/catch.wav") );
  sounds.push_back( Mix_LoadWAV("sound/dash.wav") );
...
void SDLSound::operator[](int index) {
  ...
  currentSound = index;
  Mix_VolumeChunk(sounds[index], 1);
  channels[index] = 
